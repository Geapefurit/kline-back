...
func SetQueryConds(q *ent.KPriceQuery, conds *Conds) (*ent.KPriceQuery, error) { //nolint
	...
	if conds.Time != nil {
		endat, ok := conds.Time.Val.(uint32)
		if !ok {
			return nil, fmt.Errorf("invalid endat")
		}
		switch conds.Time.Op {
		case cruder.LT:
			q.Where(kpriceent.TimeLT(endat))
		case cruder.LTE:
			q.Where(kpriceent.TimeLTE(endat))
		case cruder.GT:
			q.Where(kpriceent.TimeGT(endat))
		case cruder.GTE:
			q.Where(kpriceent.TimeGTE(endat))
		case cruder.EQ:
			q.Where(kpriceent.TimeEQ(endat))
		default:
