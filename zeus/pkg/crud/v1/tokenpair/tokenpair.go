...
func CreateSet(c *ent.TokenPairCreate, req *Req) *ent.TokenPairCreate {
	...
	if req.TokenTwoID != nil {
		c.SetTokenTwoID(*req.TokenTwoID)
	}
	if req.Remark != nil {
...
func SetQueryConds(q *ent.TokenPairQuery, conds *Conds) (*ent.TokenPairQuery, error) { //nolint
	...
	if conds.IDs != nil {
		...
		switch conds.IDs.Op {
		case cruder.IN:
			q.Where(tokenpairent.TokenOneIDIn(ids...))
		default:
	...
	if conds.TokenTwoID != nil {
		id, ok := conds.TokenTwoID.Val.(uint32)
		if !ok {
			return nil, fmt.Errorf("invalid tokentwoid")
		}
		switch conds.TokenTwoID.Op {
		case cruder.EQ:
			q.Where(tokenpairent.TokenTwoID(id))
		default:
			return nil, fmt.Errorf("invalid tokentwoid field")
		}
	}
	if conds.Remark != nil {
