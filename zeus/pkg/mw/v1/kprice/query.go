...
func (h *Handler) GetKPrice(ctx context.Context) (*kpriceproto.KPrice, error) {
	...
	err := db.WithClient(ctx, func(_ctx context.Context, cli *ent.Client) error {
		return handler.queryKPrice(cli)
	})
	if err != nil {
		return nil, err
	}
...
func (h *Handler) GetKPoint(ctx context.Context) (*kpriceproto.KPrice, error) {
	handler := &queryHandler{
		Handler: h,
	}
...
func (h *Handler) GetEarlistKprices(ctx context.Context) ([]*kpriceproto.KPrice, uint32, error) {
	...
	err := db.WithClient(ctx, func(_ctx context.Context, cli *ent.Client) error {
		...
		handler.stm.
			Offset(int(h.Offset)).
			Limit(int(h.Limit)).
			Order(ent.Asc(kpriceent.FieldTime))
		return handler.scan(_ctx)
	})
	if err != nil {
		return nil, 0, err
...
func (h *Handler) GetLatestKprices(ctx context.Context) ([]*kpriceproto.KPrice, uint32, error) {
	...
	err := db.WithClient(ctx, func(_ctx context.Context, cli *ent.Client) error {
		...
		handler.stm.
			Offset(int(h.Offset)).
			Limit(int(h.Limit)).
			Order(ent.Desc(kpriceent.FieldTime))
		return handler.scan(_ctx)
	})
	if err != nil {
		return nil, 0, err
...
func getKPointFromKPrice(ctx context.Context, cli *ent.Client, startTime, endTime uint32, kpType basetype.KPointType) ([]*kpointproto.KPointReq, error) {
	selectMinMaxSql := fmt.Sprintf(
		"SELECT token_pair_id,MIN(price) as low,MAX(price) as high FROM  kprices WHERE `time`>=%v && `time`<=%v  GROUP BY token_pair_id;",
		startTime,
		endTime,
	)
	selectOpenSql := fmt.Sprintf(
		"SELECT t1.token_pair_id,t1.price as open FROM kprices t1 INNER JOIN (SELECT MIN(`time`) as `time` ,token_pair_id FROM kprices WHERE `time`>=%v GROUP BY token_pair_id ) t2 ON t2.token_pair_id = t1.token_pair_id AND t2.`time` = t1.`time`;",
		startTime,
	)
	selectCloseSql := fmt.Sprintf(
		"SELECT t1.token_pair_id,t1.price as close FROM kprices t1 INNER JOIN (SELECT MAX(`time`) as `time` ,token_pair_id FROM kprices WHERE `time`<=%v GROUP BY token_pair_id ) t2 ON t2.token_pair_id = t1.token_pair_id AND t2.`time` = t1.`time`;",
		endTime,
	)
