...
type TokenPair struct {
	...
	// TokenTwoID holds the value of the "token_two_id" field.
	TokenTwoID uint32 `json:"token_two_id,omitempty"`
...
func (*TokenPair) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		switch columns[i] {
		case tokenpair.FieldID, tokenpair.FieldCreatedAt, tokenpair.FieldUpdatedAt, tokenpair.FieldDeletedAt, tokenpair.FieldTokenOneID, tokenpair.FieldTokenTwoID:
...
func (tp *TokenPair) assignValues(columns []string, values []interface{}) error {
	...
	for i := range columns {
		...
		case tokenpair.FieldTokenTwoID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field token_two_id", values[i])
			} else if value.Valid {
				tp.TokenTwoID = uint32(value.Int64)
			}
		case tokenpair.FieldRemark:
...
func (tp *TokenPair) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("token_two_id=")
	builder.WriteString(fmt.Sprintf("%v", tp.TokenTwoID))
	builder.WriteString(", ")
